...
def target_update(critic: Model, target_critic: Model, tau: float) -> Model:
    ...
    new_target_params = jax.tree_map(update_fn, 
                                     critic.params,
...
def update(key: PRNGKey, actor: Model, critic: Model, target_critic: Model,
           ...
           soft_critic: bool, use_sam:bool, rho:float, only_enc:bool) -> Tuple[Model, InfoDict]:
    dist, _, _ = actor(batch.next_observations)
    next_actions = dist.sample(seed=key)
    # next_actions = jnp.clip(next_actions, -1.0 + eps, 1.0 - eps)
    ...
    new_critic, info, norm = critic.apply_gradient(critic_loss_fn, use_sam, rho, only_enc)

    return new_critic, info, norm
